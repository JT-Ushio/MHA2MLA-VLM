...
from transformers import HfArgumentParser, LlamaModel, Qwen2_5_VLForConditionalGeneration, LlavaNextForConditionalGeneration, LlavaForConditionalGeneration
...
from lr_scheduler import calculate_max_steps
...
def main():
    ...
    if training_args.optimizer == "constant_with_warmup_decay":
        from lr_scheduler import create_constant_with_warmup_decay_optimizer, set_lr_scheduler_args
...
class LlavaMLASFT(SwiftSft):
    ...
    def _mha2mla(self, model):
        ...
        # ##### if stage1_path is not None, load weights and post train again
        # if self.mha2mla_args.stage1_path is not None:
    ...
    def run(self):
        ...
        if args.optimizer == "constant_with_warmup_decay" and args.optimizer_lr_scheduler_kwargs:
            from lr_scheduler import set_lr_scheduler_args
        ...
        if self.mha2mla_args.peft_train is not None:
            ...
            elif isinstance(self.model, LlavaNextForConditionalGeneration):
                for param in self.model.language_model.lm_head.parameters():
                    param.requires_grad = False
            elif isinstance(self.model, LlavaForConditionalGeneration):
                for param in self.model.language_model.lm_head.parameters():
                    param.requires_grad = False
            else:
