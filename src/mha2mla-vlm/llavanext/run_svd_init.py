import argparse
from transformers import AutoConfig, LlavaNextForConditionalGeneration
...
def main(args):
    ...
    print("init model")
    mha_model = LlavaNextForConditionalGeneration(
        config
    ).cuda()
    ...
    if isinstance(mla_model_s1, LlavaNextForConditionalGeneration):
