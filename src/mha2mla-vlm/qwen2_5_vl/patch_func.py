...
def partial_rope_mask(model_args, mha2mla_args):
    ...
    def select_mkl_frequency(mask, rope_dim_for_mla):
        """
        Select dimensions based on MKL frequency importance
        Returns:
            mask: Binary mask with 1s for the top rope_dim_for_mla dimensions by MKL
        """
        # Load pre-computed importance rankings from qk_tensor_path
        # The rankings are computed based on MKL
        with open(mha2mla_args.qk_tensor_path, "rb") as fin:    # [num_layers, num_key_value_heads, head_dim]
        ...
        k_masks = k_masks.view(k_masks.size(0), -1) # [num_layers, head_dim * num_key_value_heads]
        q_masks = q_masks.view(q_masks.size(0), -1) # [num_layers, head_dim * num_attention_heads]
        return q_masks, k_masks
    ...
    elif rope_version == "mkl":
        return select_mkl_frequency(d_head, rope_dim_for_mla)
    else:
        raise NotImplementedError(f"Invalid rope_version: {rope_version}")
