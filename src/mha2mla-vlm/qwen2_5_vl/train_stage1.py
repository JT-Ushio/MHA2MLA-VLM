...
class LlavaMLASFT(SwiftSft):
    ...
    def _mha2mla(self, model):
        ...
        if hasattr(model.config.get_text_config(), "llm_config"):   # fix for internvl
            model, q_idx, k_idx = patch_model(model, model.config.get_text_config().llm_config, self.mha2mla_args)
        else:
    ...
    def run(self):
        ...
        if self.mha2mla_args.peft_train is not None:
            freeze_non_attn_weights(self.model.model, self.mha2mla_args.peft_train)
