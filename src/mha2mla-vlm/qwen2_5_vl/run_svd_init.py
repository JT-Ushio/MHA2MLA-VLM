...
def main(args):
    ...
    print("init model")
    mha_model = Qwen2_5_VLForConditionalGeneration(config).cuda()
    # patch model and load weight
    print("patch model")
    mla_model_s1, q_idx, k_idx = patch_model(
        mha_model, mha_model.config.get_text_config(), mha2mla_args
    )
    ...
    if isinstance(mla_model_s1, Qwen2_5_VLForConditionalGeneration):
    ...
    def save_input_hook(module, input, output):
        ...
        if (
            ...
            hook_ctx.kvproj_inputs[f"{module.layer_id}.img"] += cal_s(
                input[0][hook_ctx.image_mask]
            ).detach()
            hook_ctx.kvproj_inputs[f"{module.layer_id}.text"] += cal_s(
                input[0][~hook_ctx.image_mask]
            ).detach()
        else:
            hook_ctx.kvproj_inputs[f"{module.layer_id}.img"] = cal_s(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="testing"):
            ...
            hook_ctx.image_mask = (
                (batch["input_ids"] == config.image_token_id)
                | (batch["input_ids"] == config.vision_end_token_id)
                | (batch["input_ids"] == config.vision_start_token_id)
            )
            model(**batch)
    ...
    r = model.config.num_key_value_heads * low_rank
    for layer_id in tqdm(range(config.num_hidden_layers)):
        # image
        input = S["S"][f"{layer_id}.img"] / S["image_token_acc"] / S["sample_acc"]
        print(
            "text_token_acc image_token_acc, sample_acc: ",
            S["text_token_acc"],
            S["image_token_acc"],
            S["sample_acc"],
        )
        Us, Ss, Vs = torch.linalg.svd(input.float(), full_matrices=False)
        Wk = model.model.layers[layer_id].self_attn.kv_proj.up_k.weight
        Wv = model.model.layers[layer_id].self_attn.kv_proj.up_v.weight
        W = torch.cat([Wk, Wv])
